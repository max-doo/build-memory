...
flowchart TD
    ...
    I3 --> I4["写入项目根目录 (包含 .memory/ 支撑目录)"]

    I4 --> J["4. 完善已有文件"]
    J --> J1["读取现有文件全文"]
    J1 --> J2["按需读取 reference/ 规范或 .memory/KNOWLEDGE.md"]
    J2 --> J3["对照 spec 比对差距"]
    J3 --> J4{"差距分类"}

    J4 --> |"Minor<br/>轻微措辞、过期命令"| K["直接修改并报告"]
    J4 --> |"Major<br/>角色错误、强行融合、超长"| L["明确指出违反规范之处并请求用户确认"]
...
sequenceDiagram
    ...
    participant CM as AGENTS.md / CLAUDE.md<br/>（项目规则记忆）
    participant SL as .memory/session_log.py<br/>（写入会话日志）
    ...
    U->>A: 继续优化登录模块
    A->>CM: 【自动读取】加载项目规则
    A->>SL: 【按需读取】读取 SESSION_LOG.md 历史
...
flowchart LR
    subgraph Session["一个 Session 的完整记忆流"]
        ...
        subgraph Input["输入层"]
            I1["用户指令"]
            I2["AGENTS.md & CLAUDE.md<br/>自动加载"]
        ...
        subgraph Output["输出/持久化层"]
            O1["代码变更 → Git"]
            O2["调用脚本 → SESSION_LOG.md<br/>(及 .memory/ 归档)"]
            O3["任务状态 → TODO.md"]
...
| 范围 | 路径 | 可用范围 |
|------|------|----------|
| 项目（推荐） | `~/.cursor/rules/build-memory/` | 仅当前仓库 |

全局 `<repo>/.cursor/rules/` 是 Cursor 官方支持的项目级规则目录。添加后重载工作区（`Cmd/Ctrl+Shift+P → Developer: Reload Window`）。

### Google Antigravity

| 范围 | 路径 | 可用范围 |
|------|------|----------|
| 全局 | `~/.gemini/config/skills/build-memory/` | 所有项目 |
...
- 收集可用的脚本命令
- 检查是否已存在同类文件
- （若工作区为空）主动询问用户是跳过初始化、描述项目计划，还是生成骨架
...
有意义的 Agent 会话结束后，Agent 会通过执行 `.memory/session_log.py` 脚本追加条目（包含文件锁和归档机制）：
...
- "调用 .memory/session_log.py 把我们刚才做的决策追加到 SESSION_LOG。"
...
| 文件 | 建议长度 | 核心约束 |
|------|----------|----------|
| `AGENTS.md` | 200–400 词 | 单一事实来源；包含项目所有的核心事实与规则 |
| `CLAUDE.md` | 极简入口 | 仅引用 `@AGENTS.md`；可包含极少量的行为调整 |
| `CHANGELOG.md` | 随版本增长 | 仅记录版本级变更，而非日常提交 |
| `SESSION_LOG.md` | 仅追加 | 按 Session 记录；推荐通过 `.memory/session_log.py` 写入 |
| `TODO.md` | 动态维护 | 仅包含 `Pending` / `Done`，不要设 `In progress` |
