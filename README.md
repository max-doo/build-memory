...
Initialize a baseline set of collaboration rules and tracking files for working with AI coding agents.

---

## Why this skill exists

When you repeatedly use Claude, Codex, or other AI coding assistants on a project, you tend to hit the same friction points:

- **Every new session starts from zero**—you re-explain the tech stack, naming conventions, and key directories.
- **The agent remembers the last suggestion within a session, but forgets across sessions.** Context windows are finite; there is no persistent cross-session memory.
- **Multiple people or multiple agents share one rule set, and the rules collide.** Codex's repository-level guidelines and Claude's session-level prompts get mixed into the same file.
- **Working notes scatter across chat history.** What changed, why it changed, and what's still pending becomes hard to recover after the fact.

`workspace-init` addresses these problems by creating a small set of lightweight, role-separated files that let agents ramp up faster and keep your collaboration visible and auditable.

---

## What the skill does

It inspects your workspace and creates or refines five managed files:

| File | Purpose | Audience |
|------|---------|----------|
| `AGENTS.md` | General project guidelines—directory conventions, tech stack, testing requirements, prohibitions | All agents (Claude, Codex, Cursor, etc.) |
| `CLAUDE.md` | Claude-specific session memory—key file paths, common commands, project shortcuts | Claude Code |
| `CHANGELOG.md` | Release-level change history | Developers and users |
| `SESSION_LOG.md` | Per-session collaboration log—decisions made, problems encountered | Developers |
| `TODO.md` | User-governed backlog; the agent assists but does not add items autonomously | Developers |

Each file has a clear, distinct role. This avoids the common failure mode of one file accumulating everything.

---

## How it works

### 1. Inspect first, write second

The skill never invents content. It first inspects your project:

- Top-level directory structure
- Build manifests such as `package.json`, `pyproject.toml`, `Cargo.toml`
...
For each missing file, the skill generates content from a template using project-specific facts and translates fixed text into the target language (the language is asked rather than guessed).

### 3. Refine existing files
...
The core principle: **no silent rewrites.** Existing content is respected.

### 4. Two non-negotiable rules

- **Independence.** `AGENTS.md` and `CLAUDE.md` do not reference each other. A small amount of duplicated facts between the two is acceptable—each file is self-contained for its own audience.
- **User confirmation.** Any substantive rewrite requires your explicit approval first.

---

## Why this matters for human-agent collaboration

The value is not "more automation"—it is **predictable, time-saving collaboration**:

### Reduced session startup cost

Agent context is finite, but project background is unbounded. Stable background knowledge moves into files; each session loads a few hundred words instead of starting from "what test framework do we use?"

### Separation of "rules" from "memory"

- `AGENTS.md` is **the spec**—conventions any agent should follow.
- `CLAUDE.md` is **memory**—shortcuts for how Claude in particular sees this project.

Keeping them separate means switching agents preserves the spec, while tool-specific tuning never pollutes the shared rules.

### Traceable, not replaceable

`SESSION_LOG.md` and `CHANGELOG.md` do not replace `git history`. They capture the **why**—the context behind a design decision, the path taken during a hard debugging session. These are the things `git log` does not preserve.

### The user stays in charge

`TODO.md` has only `Pending` and `Done` states; there is no `In progress`. This is intentional: the agent should not unilaterally decide what is currently in flight. It maintains a list under your control, not its own.
...
**`AGENTS.md` + `CLAUDE.md` — open-book context for the agent**

Claude Code automatically reads `CLAUDE.md` and `AGENTS.md` from your project. You no longer need to recite project background at the start of each session. When the agent's behavior drifts from expectations, the rule files usually need an update—edit them, and the next session picks up the change automatically.
...
You and the agent both maintain it, but you hold the steering wheel. The agent does not silently add an "In progress" state—what is currently in flight is your call.
