...
`SESSION_LOG.md` is the agent's recent operational log. It records session-level operational changes, file-level edits, and unresolved items for the recent 7-day window. It supports context recovery, audit of edit paths, and cross-session handoff without forcing agents to load stale long-term history.
...
First, agents should append entries with `python .memory/session_log.py` instead of editing `SESSION_LOG.md` by hand. The script obtains the current system time, uses a lock file, archives date blocks older than the recent window, and writes a structured entry.

Second, multiple changes may share a single timestamp. This aggregates one continuous operation and avoids fragmenting the log.

Third, bug-fix entries should record reusable debugging context through `lesson` only when it has future value. This is one of the log's core purposes—"do not fall into the same pit twice." Restrict entries to pitfalls with reuse value; do not log every failed attempt. Stable lessons can later be promoted to `.memory/KNOWLEDGE.md`.
...
```md
- `SESSION_LOG.md`: recent 7-day collaboration log. Use `python .memory/session_log.py` to append session notes; do not edit it manually. The script handles current time, lock retries, old-date archival, and structured formatting.
...
```md
- `SESSION_LOG.md`：最近 7 天协作日志；使用 `python .memory/session_log.py` 追加 session 记录，不要手工编辑。脚本会处理当前时间、锁重试、旧日期归档和结构化格式。
- `.memory/KNOWLEDGE.md`：长期可复用经验和决策；仅在反复问题、调试、架构决策或当前任务明显依赖项目历史经验时读取。
...
```md
- Tracking files: read `SESSION_LOG.md` for recent context, append through `python .memory/session_log.py`, use `.memory/KNOWLEDGE.md` only for durable lessons, `TODO.md` for user-governed backlog, and `CHANGELOG.md` only for release-facing changes.
```

Chinese:

```md
- 追踪文件：`SESSION_LOG.md` 记录最近协作上下文，追加记录使用 `python .memory/session_log.py`，`.memory/KNOWLEDGE.md` 仅存长期经验，`TODO.md` 记录用户授权的项目级待办，`CHANGELOG.md` 仅记录 release 变化。
```

---

## 6. Recommended build-memory Behavior

The initialization skill should follow this strategy:

1. Create `SESSION_LOG.md` and `.memory/` support files by default, unless an equivalent work log and writer already exist.
...
6. If `SESSION_LOG.md` already exists, do not overwrite it; configure the script as the recommended future write path. On first script run, it may archive parseable old date blocks older than the recent window.
...
- Create `SESSION_LOG.md` and `.memory/session_log.py` if no equivalent work/session log writer exists.
...
```text
SESSION_LOG.md  # recent 7-day session-level operations
.memory/        # writer script, long-term lessons, and archived old logs
...
```text
SESSION_LOG.md: append through `python .memory/session_log.py` after session end when files actually changed or items remain unresolved.
